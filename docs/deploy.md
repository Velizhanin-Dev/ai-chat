...
# Папка загрузок генератора превью (референсы + сгенерированные картинки).
# Монтируется в контейнер как /app/data/uploads; контейнер работает под uid 1001,
# поэтому папку надо создать заранее и отдать ему — иначе запись упадёт на EACCES.
mkdir -p data/uploads && sudo chown -R 1001:1001 data/uploads
```
