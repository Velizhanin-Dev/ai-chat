...
- 🟡 **Квоты запросов и пробный тариф** — доступ к ассистенту = тариф не истёк И квота
  ...
  **Лимит ПРОЕКТОВ** (`Plan.limits.projects`) тоже применяется — см. «Проекты» выше
  ...
  - **Пробный тариф — ВСЕГО на 1 час.** Регистрация (email и OAuth) выдаёт `plan="start"` +
    ...
    час — `Plan.limits.requests` тарифа start (дефолт 18, правится в админке). Заново триал
...
- 🟢 **Фикс «плоских» ответов GLM (markdown-форматирование).** Общий system-промпт
  ...
  для GLM** (`systemText`): явно разрешает markdown (жирный на ключевом, `##`-подзаголовки,
  списки для перечислений, пустые строки между блоками), но с оговоркой «не сухая
  презентация, живая речь от первого лица, меру держи». **Добавляется только на
