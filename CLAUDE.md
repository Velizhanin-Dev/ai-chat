...
- 🟢 **Бриф клиента + карта харизмы (DISC)** — мастер «Знакомство перед стартом» в 2 шага
  ...
  - Шаг **«О себе»** — три под-режима (`discMode` в `BriefFlow`: `choose`/`test`/`manual`):
    ...
    - **`test`** — короткий DISC-тест (10 forced-choice вопросов под съёмку), по одному
      вопросу; варианты — кликабельные `Radio.Card` с **автопереходом** после выбора.
      Прогресс-бар показываем только тут (не на `choose`/`manual`).
    - **`manual`** — ручной выбор архетипа: `Radio.Group` из 7 типов (код + наше название
      + краткая характеристика), кнопка **«Готово»** (`disabled` без выбора) → сохраняет
      бриф с выбранным `disc` напрямую, без скоринга.
    Общий финиш вынесен в `submitBrief(disc)`; `finish(answers)` (тест) считает `scoreDisc`
    и зовёт его, `manual` зовёт с выбранным типом. `goBack` из `test`(вопрос 1)/`manual`
    возвращает на `choose`, из `choose` — к последнему вопросу «о проекте». Черновик:
    на восстановлении step=1 → `test`, если есть ответы, иначе `choose`.
