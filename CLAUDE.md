...
- 🟡 **Генератор превью (раздел «Генератор превью», заменил заглушку «Разборы»).** Пер-проектный
  раздел `/{projectId}/thumbnails` (роут `src/app/[projectId]/(locked)/thumbnails/`), компонент
  [ThumbnailStudio](src/components/Thumbnails/ThumbnailStudio.tsx). Пока **админ-онли**: страница —
  в route-group `(locked)` (серверный гвард), API — флагом `THUMBNAILS_ADMIN_ONLY` в
  [thumbnails-server.ts](src/lib/thumbnails-server.ts). Открыть всем = снять флаг + вынести роут
  из `(locked)` (гейты квоты/тарифа уже написаны и включатся сами). **Не протестировано вживую**
