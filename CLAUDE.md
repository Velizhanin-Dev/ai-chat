...
- 🟢 **Чат-обвязка только на `/chat`.** `AppShell` навешивает сайдбар/шапку ТОЛЬКО на
  `/chat` (`pathname === "/chat" || startsWith("/chat/")`); всё остальное (лендинг, auth,
  `/admin`, `/legal`, `/brief`, `/payment`, а также 404/500) рендерится «голым», своим
  layout. Раньше был список `BARE_ROUTES` — заменён на это правило (чтобы и неизвестные
  URL/ошибки шли без чат-обвязки).
