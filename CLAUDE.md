...
- 🟡 **Настройки (модалка)** — `components/Settings/SettingsModal.tsx`, открывается из
  ...
  - **Биллинг:** карточки трёх тарифов (Пробный 0 ₽ — 1 день / 18 запросов; Базовый
    4 000 ₽/мес — 3 КП, 30 сценариев, 90 шортсов; Максимальный 10 000 ₽/мес — всё без
    лимита). id'шники `PlanId` (`start`/`blogger`/`studio`) исторические, переименованы
    только лейблы (`PLAN_LABEL` в `authSlice`). Переход на тариф — **заглушка**: онлайн-
    оплаты нет, `settings.plan` не меняем, показываем `Alert` про ручной перевод
    (`handleChoosePlan` → `pendingPlan`). Тарифы синхронны с лендингом (`Pricing.tsx`).
