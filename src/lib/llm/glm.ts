...
// ── GLM-специфичная директива форматирования ────────────────────────────────
// Общий system-промпт задаёт «связные абзацы, не простыни-буллеты» (голос
// Велижанина). Claude балансирует структуру и живую речь сам, а GLM понимает эту
// установку буквально и выдаёт «плоские» монолитные ответы без выделений и
// иерархии — особенно на тематических/разборных запросах. Поэтому ТОЛЬКО для GLM
// добавляем в конец system блок, который явно разрешает и просит markdown-разметку
// (заголовки, жирный на ключевом, списки для перечислений) — не ломая голос.
// Ответы рендерятся как markdown (components/Chat/Markdown.tsx), так что разметка
// отобразится корректно.
const GLM_FORMATTING = `# Оформление ответа (markdown)

Ответ рендерится как красивый markdown — заголовки, цитаты, списки, таблицы, жирный текст стилизованы и выглядят аккуратно. Пользуйся ВСЕМ набором, чтобы длинный ответ читался как хорошо свёрстанный разбор, а не как стена текста:

- **Заголовки разделов.** Если в ответе несколько смысловых частей (например: разбор → варианты → что дальше) — дай каждой заголовок \`## Название\`, подпунктам — \`### \`. Так длинный ответ становится навигируемым, глаз цепляется за структуру.
- **Цитаты для речи и готовых фраз.** Текст, который человек произносит на камеру (хук, реплика, закадровый текст, пример фразы, цитата) — выноси в блок-цитату через \`> \`. Сразу видно: вот это — то, что говоришь вслух. Например:
  > «Эти калоши живут ДОЛЬШЕ, чем твоя рыбалка»
- **Жирным** выделяй ключевые мысли, термины и названия блоков (хук, CTA, формат) — то, за что цепляется глаз.
- **Списки** — для перечислений (варианты, идеи, шаги, пункты структуры): нумерованный, если есть порядок; маркированный, если нет. Каждый пункт с новой строки.
- **Таблица** — если реально сравниваешь варианты по колонкам. Между разделами — пустая строка; крупные части можно отбивать \`---\`.

Но меру держи, и это ГЛАВНОЕ: разметка ОФОРМЛЯЕТ мысль, а не заменяет её. Связный разбор пиши абзацами; заголовки и цитаты ставь там, где реально есть отдельный раздел или произносимая реплика, — а не на каждой строке. И это ПО-ПРЕЖНЕМУ моя живая речь от первого лица, с моими интонациями и матом к месту — красивая вёрстка не должна превращать ответ в сухую презентацию или инструкцию.`;
...
export const glmStrategy: LlmStrategy = {
  ...
  async *stream({ system, messages, route, routeMs, meta }: StreamArgs) {
    ...
    const systemText = [
      ...
      ...(route.category !== "chat" ? [GLM_FORMATTING] : []),
