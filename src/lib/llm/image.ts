...
export async function generateImage({
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (!resp.ok) {
      ...
      // ⚠️ Блок модерации — НЕ наша поломка и ретраем не лечится: Gemini
      // отказывается рисовать тему (чаще всего политика, персоны, катастрофы).
      // Сырой JSON от OpenRouter в интерфейсе читался как «всё сломалось»,
      // хотя правильное действие у человека одно — смягчить формулировки.
      // Ловили вживую: «Кто наживётся на крахе России» → 400 content moderation.
      if (/moderation|blocked|safety|prohibited/i.test(errText)) {
        throw new Error(
          "Google отказался рисовать эту тему (модерация содержания: политика, реальные персоны и катастрофы у него под фильтром). Квота не потрачена. Смягчи текст на превью и описание ролика — без имён политиков и слов про крах/войну — и попробуй ещё раз. Либо смени модель генерации в админке на не-Google."
        );
      }
