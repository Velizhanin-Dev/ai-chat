...
export async function POST(req: Request) {
  ...
  try {
    ...
    const videoLines = videos.length
      ? videos
          .map((v) => `- «${v.title}» — ${v.viewCount} просмотров`)
          .join("\n")
      : "(роликов пока нет)";

    // Описания первых роликов дают контекст про продукт/оффер (ссылки, лид-магниты).
...
Ссылка: ${channel.customUrl ?? "—"}
Подписчиков: ${channel.hiddenSubscriberCount ? "скрыто" : channel.subscriberCount}
Роликов: ${channel.videoCount}, суммарных просмотров: ${channel.viewCount}
Описание канала:
«${channel.description ? channel.description.slice(0, CHANNEL_DESC_MAX) : "(пустое)"}»

ПОСЛЕДНИЕ РОЛИКИ (название — просмотры):
${videoLines}

Свежие темы: ${descriptions.slice(0, VIDEO_DESC_MAX) || "—"}
...
Главное правило: не выдумывай. Если данных на поле не хватает — верни по нему пустую строку.`,
      ...
      meta: { userId: user.id, conversationId: null },
    })) {
      full += token;
    }

    const autofill = extractAutofill(full, channel.title);
    if (!autofill) {
      console.error("[brief autofill] parse failed:", full.slice(0, 300));
