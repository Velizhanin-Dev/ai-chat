...
function buildSystem(
  ...
  // Эталон контент-плана — статичный, под запрос «контент-план». Ставим ПЕРЕД
  // переменным хвостом (как форматы) и кэшируем: короткий видео-формат и контент-план
  // взаимоисключимы, поэтому лишний кэш-брейкпоинт не появляется.
  ...
  // Это те самые «мои формулировки, копируй дословно» — а модель сильнее всего
  // весит то, что стоит последним перед вопросом (recency). Раньше эти блоки шли
  // в середине (перед дисциплиной/брифом/«о себе»), и между эталоном и вопросом
  // вклинивалось несколько блоков — модель хуже копировала. Все они без кэша
  // (разные под каждый запрос), поэтому перестановка внутри некэшируемого хвоста
  // ЦЕНУ не меняет и кэш-префикс (хребет + форматы) не ломает.
  if (route.tgClosed) {
