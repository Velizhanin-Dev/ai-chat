...
export async function GET(req: Request) {
  ...
  try {
    ...
    const models = (data.data ?? [])
      // ⚠️ Варианты «:batch» вычищаем: это асинхронная пакетная обработка БЕЗ
      // стриминга — выбор такой модели в селекторе сломал бы чат (мы стримим
      // токены живому человеку), а у DeepSeek batch ещё и дороже обычного.
      // «~»-алиасы («latest», редиректы) тоже не показываем: настройка должна
      // указывать на конкретную модель, а не плыть за чужими релизами.
      .filter((m) => !m.id.includes(":batch") && !m.id.startsWith("~"))
      .map((m) => ({
        id: m.id,
        name: m.name || m.id,
